...
if [ ${param} == "balance" ]; then
 ...
 msg "\n$(cat $tdir/total_final3 $tdir/footer |column -t $(man -P cat column| grep "\-o" && printf '%s' -o ' | ') |sed -E 's/\|/ \| /g; s/^/\\033\[0;34m/g; s/ (-[0-9\.]+%)/ \\033\[0;31m\1\\033\[0;34m/g' |tee $tdir/total_final4)"
 ...
 if [[ $exchange == "all" ]] ; then
  ...
  msg "\n\033[0;34m$(cat $tdir/total_per_exchange |column -t $(man -P cat column| grep "\-o" && printf '%s' -o ' | ') |sed 's/|/ | /g' |grep --color ".*")\033[0m"
 fi
 if [ ! -z $fiat_deposits ]; then
  ...
  msg "\n$(cat $tdir/total_result |column -t $(man -P cat column| grep "\-o" && printf '%s' -o ' | ') |sed -E 's/\|/ \| /g; s/^/\\033\[0;34m/g; s/ (-[0-9\.]+%)/ \\033\[0;31m\1\\033\[0;34m/g')\033[0m"
 fi
 exit
fi
